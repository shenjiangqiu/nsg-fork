...
class TaskQueue {
public:
  inline void push(std::function<void()> task) { tasks.push(task); }
...
inline void taskProducer(TaskQueue &taskQueue, int n, std::mutex &cvMtx,
...
inline void taskConsumer(TaskQueue &taskQueue, std::mutex &cvMtx,
                         std::condition_variable &cv, bool &ready,
...
/// the demo to show how to use the task queue
int demo() {
