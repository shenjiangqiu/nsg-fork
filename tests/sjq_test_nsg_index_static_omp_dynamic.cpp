...
#include <efanna2e/index_nsg_sjq_static_omp_dynamic.h>
...
int main(int argc, char **argv) {
  if (argc != 10) {
    std::cout << argv[0] << " data_file nn_graph_path L R C save_graph_file T B Sync"
              << std::endl;
    exit(-1);
  }
  ...
  unsigned Sync = (unsigned)atoi(argv[9]);
  ...
  sjq_static_omp_dynamic::IndexNSG index(dim, points_num, efanna2e::L2,
                                         nullptr);
  ...
  for (unsigned traversal_idx = 1; traversal_idx <= 3; traversal_idx++) {
    for (unsigned round = 0; round < 5; round++) {
      ...
      // reorder the traversal_sequence
      unsigned nd_rounded_ = (index.nd_ + T - 1) / T * T;
      unsigned *traversal_sequence_reordered = new unsigned[nd_rounded_];
      index.nd_rounded = nd_rounded_;
      sjq_static_omp_static::distribute_traversal_sequence(
          traversal_sequence, traversal_sequence_reordered, T, index.nd_,
          nd_rounded_);
      ...
      index.Build_static(points_num, data_load, paras,
                         traversal_sequence_reordered);
